...
def main():
    ...
    print('🚀 Démarrage Entraînement H-JEPA (V3) - FINAL T=5 BPTT + DECODER LOSS')
    ...
    if os.path.exists('checkpoints/agent_h_jepa.pth'):
        ...
        print('✅ AutoEncoder chargé. Perception gelée pour entraînement de la dynamique T=5.')
    ...
    for episode in range(num_episodes):
        ...
        for update in range(20):
            ...
            try:
                ...
                # BPTT sur T=5
                s_preds, _ = world_model.forward_seq(s_0, a_seq_onehot)
                
                # 1. MSE sur le latent (T=5)
                mse = (s_preds - s_next_target) ** 2
                agent_mask = x_next_seq_batch[:, :, 3:4]
                weight_mask = torch.ones_like(mse) + 100.0 * agent_mask
                loss_latent = (mse * weight_mask).mean()
                
                # 2. MSE sur le décodeur explicite (T=5)
                # s_preds est [B, T, 16, 10, 10]. On l'aplatit pour le passer dans le décodeur
                s_preds_flat = s_preds.view(B*T_s, 16, 10, 10)
                agent_preds_flat = world_model.decode_agent(s_preds_flat)
                agent_preds = agent_preds_flat.view(B, T_s, 1, 10, 10)
                
                agent_mask_float = agent_mask.float()
                weight_mask_decoder = torch.ones_like(agent_mask_float) + 100.0 * agent_mask_float
                loss_recon = ((agent_preds - agent_mask_float)**2 * weight_mask_decoder).mean()
                
                # Loss globale = latent_loss + decoder_loss pour forcer l'agent à rester correct sur 5 pas
